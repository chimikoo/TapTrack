...
export default function DashboardScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      const { data } = await axios.get(
        `${TAP_TRACK_URL}/eod/receipts/date-range`,
        {
          params: {
  ...
  const totalFoodSold = receipts.reduce((acc, receipt) => {
    const starterItems = receipt.items.filter(
    ...
    const dessertItems = receipt.items.filter(
  ...
  const totalDrinksSold = receipts.reduce((acc, receipt) => {
    const drinkItems = receipt.items.filter(
      ...
      <ScrollView className="flex-1 pt-5 pl-2 pr-2">
        <View className="bg-primary-lighter shadow p-2">
          <View className="flex-row justify-between mb-4">
            <TouchableOpacity
              className="flex-row items-center p-2 border border-black rounded"
          ...
          <View className="bg-primary-lighter">
            ...
            <View className="flex-row justify-between gap-2 mt-1">
              ...
              <View className="justify-center flex-1 bg-primary-light p-3">
                ...
                <Text className="text-center text-xs">{totalCashRevenue}€</Text>
              </View>
              <View className="justify-center flex-1 bg-primary-light p-3">
                <Text className="text-center font-bold text-xs">Card</Text>
                <Text className="text-center text-xs">{totalCardRevenue}€</Text>
              </View>
              <View className="justify-center flex-1 bg-primary-light p-3">
            ...
            <View className="flex-row justify-between mt-4">
              <View className="flex-1 mr-2 bg-primary p-2">
                <Text className="text-center font-bold text-xs">Food sold</Text>
