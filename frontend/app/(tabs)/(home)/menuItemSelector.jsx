  ...
  const fetchMenuItems = async (category) => {
    ...
    try {
      ...
      if (category === 'beverage') {
        response = await axios.get('https://application-server.loca.lt/users/menu-items/beverages', {
          params: {
            name,
            price,
            sort: sortBy,
        ...
        response = await axios.get('https://application-server.loca.lt/users/menu-items/foods', {
          params: {
            category,
            name,
            price,
    ...
    <SafeAreaView className="flex-1 bg-primary-lighter p-4">
      ...
      <View className="flex flex-row justify-between items-center mb-4 space-x-2">
        ...
        <CustomButton
          ...
          handlePress={() => { /* Implement filter logic */ }}
        />
      </View>
      <ScrollView className="flex-1">
        {loading ? (
          ...
          menuItems.map((item, index) => (
            <View key={item._id} className="flex flex-col mb-2 border-b border-gray-300 pb-2">
              <Text className="w-full font-bold text-md">{item.name}</Text>
              {category === 'beverage' && item.sizesPrices ? (
                item.sizesPrices.map((sp, spIndex) => (
                  <View key={spIndex} className="flex flex-row justify-between items-center mt-2">
      ...
      <View className="w-full flex items-center mt-4">
        <CustomButton
          ...
          handlePress={() => { /* Implement add to order logic */ }}
        />
      </View>
      <View className="flex flex-row justify-between items-center mb-4">
        <TouchableOpacity>
